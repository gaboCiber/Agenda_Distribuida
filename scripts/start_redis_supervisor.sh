...
echo "                   docker exec $REDIS_C_NAME redis-cli INFO replication | grep role"
echo "6. To simulate a failure, stop Redis A: docker stop $REDIS_A_NAME"
echo "7. Observe supervisor logs for failover. One replica should become master: docker logs -f $REDIS_SUPERVISOR_NAME"
...
echo "10. Test multiple replica failures: stop multiple Redis nodes and observe failover behavior."
