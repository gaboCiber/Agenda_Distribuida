...
func main() {
	...
	// Initialize handlers
	authHandler := handlers.NewAuthHandler(redisClient, cfg.JWT.Secret, cfg.JWT.Expiration, logger)
	eventHandler := handlers.NewEventHandler(redisClient, logger)
	groupHandler := handlers.NewGroupHandler(redisClient, logger)
